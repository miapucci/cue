...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!submissionId || rating == null)
            throw validation('submissionId and rating required');
        if (rating < 1 || rating > 5)
            throw validation('rating must be 1–5');
        const sub = await findSubmissionById(submissionId);
        if (!sub)
            throw notFound('Submission not found');
        if (sub.status !== 'approved')
            throw validation('Can only review approved submissions');
