...
const router = Router();
function paramSubmissionId(req) {
    const p = req.params.submissionId;
    return Array.isArray(p) ? p[0] : p;
}
router.get('/thread/:submissionId', async (req, res, next) => {
    try {
        ...
        if (sub.talent_id !== userId && brief.creator_id !== userId)
            throw forbidden('Not part of this thread');
...
router.post('/', async (req, res, next) => {
    try {
        const { submissionId, body } = req.body;
        if (!submissionId || !body?.trim())
            throw validation('submissionId and body required');
        const userId = req.userId;
        const sub = await findSubmissionById(submissionId);
        if (!sub)
            throw notFound('Submission not found');
