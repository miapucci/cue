...
import type { PATCHSubmissionRequest } from '../types.js';
...
router.post('/:id/upload', upload.single('video'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (sub.talent_id !== userId) throw forbidden('Not your submission');
    const file = req.file;
    if (!file) throw validation('video file required');
...
router.patch('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const submissionId = paramId(req);
    const userId = req.userId!;
    const body = req.body as PATCHSubmissionRequest;
    if (!body.status) throw validation('status required');
    const sub = await findSubmissionById(submissionId);
    if (!sub) throw notFound('Submission not found');
