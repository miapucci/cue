...
import {
  ...
  findBriefsByCreator,
  findLiveBriefs,
  countSubmissionsForBrief,
  briefRowToBrief,
  countCreatorLiveBriefs,
  findSubmissionsByBrief,
  findSubmissionByBriefAndTalent,
  insertSubmission,
  findSubmissionById,
  submissionRowToSubmission,
  updateBriefPaymentStatus,
  updateBriefPaymentIntent,
} from '../db/index.js';
...
import type { CreateBriefRequest } from '../types.js';
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const body = req.body as CreateBriefRequest;
...
router.get('/:id/submissions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (brief.creator_id !== userId) throw forbidden('Not the brief creator');
...
router.post('/:id/claim', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const brief = await findBriefById(briefId);
    if (!brief) throw notFound('Brief not found');
    if (brief.status !== 'live') throw validation('Brief is not open for claims');
    const existing = await findSubmissionByBriefAndTalent(briefId, talentId);
    if (existing) throw validation('You already claimed this brief');
    const id = uuidv4();
...
router.get('/:id/can-view-submissions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const briefId = paramId(req);
    const userId = req.userId!;
    const row = await findBriefById(briefId);
    if (!row) throw notFound('Brief not found');
    if (row.creator_id !== userId) throw forbidden('Not the brief creator');
    ...
    if (row.payment_intent_id) {
      ...
      if (status === 'pending') {
        ...
        if (pi.client_secret) return res.json({ canViewSubmissions: false, clientSecret: pi.client_secret });
      }
    }
