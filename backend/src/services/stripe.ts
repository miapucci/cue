...
export interface CreatePaymentIntentParams {
  ...
  amountCents: number;
  currency: string;
  creatorStripeCustomerId?: string | null;
}
 ...
 */
export async function getPaymentIntentStatus(paymentIntentId: string): Promise<'pending' | 'captured' | 'failed'> {
  const stripe = getStripe();
  const pi = await stripe.paymentIntents.retrieve(paymentIntentId);
  if (pi.status === 'succeeded') return 'captured';
  if (pi.status === 'requires_capture') return 'captured'; // authorized, will capture
...
export async function releasePaymentToTalent(params: {
  briefId: string;
  submissionId: string;
  talentStripeConnectAccountId: string | null;
  amountCents: number;
  currency: string;
  transferGroup: string;
}): Promise<void> {
  const r = await getPool().query('SELECT payment_intent_id, payment_status FROM briefs WHERE id = $1', [params.briefId]);
  const brief = r.rows[0] as { payment_intent_id: string | null; payment_status: string } | undefined;
  if (!brief || brief.payment_status !== 'captured') return;
