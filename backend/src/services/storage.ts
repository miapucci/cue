...
export function saveSubmissionVideo(submissionId: string, buffer: Buffer, mimeType: string): string {
  ensureUploadDir();
...
export function saveSampleClip(userId: string, buffer: Buffer, mimeType: string): string {
  ensureUploadDir();
  const ext = mimeType.includes('mp4') ? 'mp4' : 'mov';
